      ...
      if (token) {
        await axios.post(
          `${API_BASE_URL}/api/analytics/custom`,
          {
            ...
            keyFile: parsed, // <-- send as keyFile
          },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        toast.success("Analytics added successfully", { position: "top-right" });
      } else {
        console.log("Parsed analytics JSON:", { keyFile: parsed });
                  ...
                  <form onSubmit={handleAddAnalyticsSubmit} className="space-y-3">
                    <div>
                      ...
                      <textarea
                        ...
                        className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-sm px-3 py-2 h-36 resize-y focus:ring-2 focus:ring-sky-500"
