...
const ProfileofUser = ({ collapsed, onLogout }) => {
      ...
      >
        <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 px-2">
          <div className="bg-gray-50 dark:bg-slate-800 px-2 max-w-2xl w-full max-h-[90vh] overflow-y-auto rounded-lg">
            ...
            <form onSubmit={handleSubmitEdit} className="pb-4">
              <div className="grid grid-cols-1 gap-6">
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1">Email</label>
              ...
              <div className="mt-8 flex justify-end space-x-3">
                ...
                <button
                  type="submit"
                  className="px-6 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#ff8633] hover:bg-[#e67328] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  disabled={isSaving}
                >
                  {isSaving ? 'Saving...' : 'Save Changes'}
                </button>
  ...
  const loadUserProfile = async () => {
    try {
      ...
      if (allUsers && typeof allUsers === 'object') {
        ...
        // If response is not an array but has user data
        else if (!Array.isArray(allUsers)) {
          // If it's a single user object
          if (allUsers.id) {
            allUsers = [allUsers];
          } else {
            throw new Error('Invalid response format: Expected user data');
          }
        }
      }
      ...
      if (!matchedUser) {
        // Try finding by ID only if username match fails
        const userById = allUsers.find(user => user.id === storedUserId);
        if (userById) {
          console.warn('User found by ID but username mismatch');
      ...
      // Don't logout immediately, give user a chance to retry
      setApiError(errorMessage);
    } finally {
      setLoading(false);
    }
  ...
  const user = currentUser ? {
    ...
    lastLogin: 'Recently',
    avatar: currentUser.photo || 'https://randomuser.me/api/portraits/men/32.jpg',
    bio: currentUser.about || `User with username ${currentUser.username || 'Unknown'}`,
    skills: Array.isArray(currentUser.skills) ? currentUser.skills : 
             (currentUser.skills ? [currentUser.skills] : ['User Management', 'Profile Editing']),
    phoneNumber: currentUser.phoneNumber || 'Not provided',
    assignedWork: currentUser.assignedWork || 'No assigned work',
    statusOfWork: currentUser.statusOfWork || 'Unknown',
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-slate-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            ...
            className="px-6 py-2 bg-[#ff8633] hover:bg-[#e67328] text-white rounded-lg font-medium transition-colors"
          >
            Retry
          </button>
        </div>
              ...
              <div className="w-full max-w-4xl dark:bg-slate-800 rounded-xl overflow-hidden">
                ...
                <div className="p-6 md:p-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                      <div>
                        <h2 className="text-xl font-semibold text-gray-800 mb-4 pb-2 border-b border-gray-200 dark:text-gray-400 dark:border-gray-600">
                          About
                        </h2>
                        <p className="text-gray-600 dark:text-gray-400">{user.bio}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {user.skills.length > 0 ? (
                            ...
                            <span className="text-gray-500 dark:text-gray-400 italic">No skills added yet</span>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
                    <button
                      onClick={() => setEditPanelOpen(true)}
                      className="px-6 py-2 bg-[#ff8633] hover:bg-[#e67328] text-white rounded-lg font-medium transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                      disabled={!user.id}
                    >
                      Edit Profile
                    </button>
