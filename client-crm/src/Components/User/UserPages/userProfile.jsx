...
const ProfileofUser = ({ collapsed, onLogout }) => {
  ...
  const EditProfilePanel = ({ profile, onClose, onSave }) => {
    ...
    return (
      <div ref={panelRef} className="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-slate-800 shadow-xl z-50">
        <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 px-2">
          <div className="bg-gray-50 dark:bg-slate-800 px-2 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-400">Edit Profile</h2>
              <button onClick={onClose} className="text-gray-500 dark:text-gray-100 hover:text-gray-700">
                ✕
              </button>
            </div>
  ...
  const handleSaveUserProfile = async (updatedUser) => {
    try {
      ...
      if (!updatedUser.id) throw new Error('User ID is missing. Cannot update profile.');
      ...
      const matchedUser = allUsers.find(u => String(u.id) === String(updatedUser.id));
      setCurrentUser(matchedUser);
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      try {
        ...
        if (!token || !storedUserId) throw new Error('Missing authentication data');
        ...
        const matchedUser = allUsers.find(user =>
          String(user.id) === String(storedUserId) &&
          user.username === storedUsername
        );
  ...
  return (
    <>
      <UserHeader onToggleSidebar={toggleSidebar} />
      <UserSidebar isOpen={isSidebarOpen} onClose={closeSidebar}>
        <div className={cn("min-h-screen", collapsed ? "md:ml-[70px]" : "md:ml-[0px]")}> 
          <div className="max-w-6xl mx-auto p-8">
            <h1 className="text-2xl font-bold">{user.name}</h1>
            <p>{user.email}</p>
            <button onClick={() => setEditPanelOpen(true)}>Edit Profile</button>
          </div>
