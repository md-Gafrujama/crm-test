...
//       // Refresh user data using the new API
...
//         // Validate user data matches stored credentials
...
const ProfileofUser = ({ collapsed, onLogout }) => {
  ...
  const handleSaveUserProfile = async (updatedUser) => {
    try {
      ...
      // Also refresh user info for profile image
      await loadUserInfo();
  ...
  // New function to load user info from your API
  const loadUserInfo = async () => {
    try {
      const token = localStorage.getItem('token');
      
      const response = await axios.get(`${API_BASE_URL}/api/allUser/info/`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      // Don't show error toast for user info as it's not critical
      // Just log the error and continue with default image
    }
  };
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      try {
        ...
        // Load both user data and user info in parallel
        const [userDataResponse] = await Promise.all([
          axios.get(`${API_BASE_URL}/api/usersData`, {
            headers: { 'Authorization': `Bearer ${token}` }
          }),
          loadUserInfo() // This will set userInfo state
        ]);

        // Handle the user data response structure
        if (!userDataResponse.data.success || !userDataResponse.data.data || !userDataResponse.data.data.user) {
          throw new Error('Invalid response structure from server');
        }
  ...
  const getUserAvatar = () => {
    // First try to get photo from userInfo API
    if (userInfo && userInfo.photo) {
      return userInfo.photo;
    }
    
    // Then try from currentUser (existing API)
    if (currentUser && currentUser.photo) {
      return currentUser.photo;
    }
    
    // Finally fallback to random image
    return 'https://randomuser.me/api/portraits/men/32.jpg';
  };

  // Function to get user name with fallback to userInfo
  const getUserName = () => {
    // Try to get name from userInfo API first
