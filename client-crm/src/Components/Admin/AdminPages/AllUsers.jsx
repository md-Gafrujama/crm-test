...
const OptimizedUserDashboard = ({ collapsed }) => {
  ...
  // SIMPLIFIED: Helper function to determine if user is active based on statusOfWork field only
  const isUserActive = useCallback((user) => {
    // Only check statusOfWork field - this is the primary status indicator
    if (user.hasOwnProperty('statusOfWork')) {
      if (typeof user.statusOfWork === 'string') {
        ...
        // Return true only if explicitly "active"
    ...
    // If no statusOfWork field, default to false (inactive)
    return false;
  }, []);
