...
const ClockIcon = lazy(() => import("lucide-react").then((module) => ({ default: module.Clock })));

const RecruiterRegister = () => {
  const [step, setStep] = useState(1); // 1 = form, 2 = image upload, 3 = OTP verify, 4 = pending approval
  const [otp, setOtp] = useState("");
  const [selectedImage, setSelectedImage] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);
  ...
  // Company types dropdown options
  const companyTypes = [
  ...
  // Step 3: Handle OTP verification and submit for approval
  const handleOtpSubmit = async (e) => {
    e.preventDefault();
    console.log("OTP form submitted with OTP:", otp);
    ...
    try {
      ...
      const { data } = await axios.post(
        `${API_BASE_URL}/api/companyOTP/verifyOTPAndSubmitForApproval`,
        formDataToSend,
        {
          headers: { 
            'Content-Type': 'multipart/form-data',
      ...
      console.log("OTP Verification and Submission Response:", data);

      if (data.success) {
        console.log("OTP verified and submitted for approval successfully");
        ...
        toast.success("OTP verified! Your registration is now pending approval.");
  ...
  const checkApprovalStatus = async () => {
    ...
    try {
      const { data } = await axios.get(
        `${API_BASE_URL}/api/company/checkApprovalStatus/${pendingApprovalData.requestId}`,
        {
          headers: { "Content-Type": "application/json" },
        }
      );
      ...
      if (data.success && data.status === 'approved') {
        ...
        toast.error("Your registration was rejected. Please contact support for more information.");
      }
    } catch (error) {
        ...
        <div className="w-full lg:w-1/2 p-8 md:p-12 overflow-y-auto">
          ...
          {step === 1 && (
            <form className="space-y-4" onSubmit={handleFormSubmit}>
              ...
              {/* Company Name and Company Type - Side by side */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Company Name</label>
                  <input
                ...
                <div>
                  ...
                  <select
                    ...
                    required
                    className={`w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 ${
                      errors.companyType ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-[#ff8633]"
                    }`}
                  >
            ...
            <div className="text-center space-y-6">
              <div className="flex justify-center">
                <Suspense fallback={<div className="w-16 h-16" />}>
                  <ClockIcon className="h-16 w-16 text-[#ff8633]" />
                </Suspense>
              </div>
              ...
              <div className="space-y-4">
                ...
                {pendingApprovalData && (
                  <div className="bg-gray-50 rounded-lg p-4 text-left space-y-2">
                    ...
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">Company:</span> {pendingApprovalData.companyName}
                    </p>
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">Email:</span> {pendingApprovalData.email}
                    </p>
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">Request ID:</span> {pendingApprovalData.requestId}
                    </p>
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">Submitted:</span> {new Date(pendingApprovalData.submittedAt).toLocaleString()}
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600">
                    After approval, you'll be able to log in and access all recruiter features.
                  </p>
                </div>
              </div>
              ...
              <div className="flex space-x-4">
                ...
                <button
                  ...
                  className="flex-1 bg-[#ff8633] text-white py-3 rounded-md font-semibold hover:bg-[#e47b17] transition-colors"
                >
                  Go to Login
                </button>
              </div>
